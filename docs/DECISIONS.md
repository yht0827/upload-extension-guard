...
**선택지:** E2E 테스트 / 통합 테스트 / 단위 테스트 / 혼합

**결정:** 계층별 단위 테스트 + 동시성 테스트

**이유:**
- 프로젝트 규모에 맞는 적정 테스트 범위
- E2E는 오버엔지니어링, 단위 테스트로 충분한 커버리지

**계층별 테스트 방식:**

| 계층 | 방식 | 이유 |
|------|------|------|
| Entity | 순수 단위 테스트 | 도메인 로직 검증 (정규화, 검증) |
| Repository | @DataJpaTest | 쿼리 메서드 동작 검증 |
| Service | Mock 단위 테스트 | 비즈니스 로직 격리 테스트 |
| Controller | @WebMvcTest | HTTP 요청/응답 검증 |

**동시성 테스트:**
- ExecutorService로 멀티스레드 환경 시뮬레이션
- 200개 제한이 동시 요청에서도 보장되는지 검증
- 비관적 락 동작 확인
...
**선택지:** 실제 DB / 인메모리 DB / Mock

**결정:** 계층에 따라 혼합

| 계층 | 방식 | 이유 |
|------|------|------|
| Repository | H2 인메모리 | 실제 DB 동작 검증 필요 |
| Service | Mockito Mock | 빠른 실행, Repository 격리 |
| Controller | MockMvc + Mock Service | HTTP 계층만 테스트 |

**이유:**
- Repository는 실제 쿼리 동작 확인 필요
- Service/Controller는 Mock으로 빠른 피드백
